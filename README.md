...
2. **With one exception, business jets operated after 7:00.** 34 jet operations by 19 distinct jets; the only pre-7:00 jet movements were one Pilatus PC-24's 3-minute stop on August 3. Earliest jet movement each morning excluding that stop: 7:03, 7:12, 7:20, 7:29, 7:37, 7:41, 7:59, 8:11. Two jets powered up well before 7:00 (6:16 and 6:41) and departed after it.
...
4. **The other pre-7:00 operations:** 8 turboprop movements (Pilatus PC-12s and Epic E1000s departing for the Bay Area, the Central Valley, and Arizona between 6:13 and 6:44), 1 piston departure, and 1 medevac helicopter arrival at 5:12 am.
...
6. **Not all early activity is KTRK traffic.** Across the week, eleven aircraft crossed within 10 nm of the field below 13,000 ft without landing, up to three in a morning: transiting light aircraft 2,400 to 4,500 ft above the valley, turboprops climbing through, and Reno airline arrivals descending 17 nm east. An operations log for KTRK does not capture these.
7. **Tracking visibility varies by aircraft.** 7 of the 59 operators are enrolled in the FAA's [LADD program](https://www.faa.gov/pilots/ladd), which asks commercial tracking sites such as FlightAware not to display them; the 7 include two of the four aircraft with the most pre-7:00 operations. None used the stronger PIA anonymized-registration program. The raw ADS-B broadcast remains public regardless, which is what this analysis uses. Aircraft ownership entities (LLCs and trusts) are standard registration practice and are reported here as they appear in the FAA registry.

Scope notes: one summer week; ADS-B-equipped aircraft only, so light-aircraft counts are floors; this study does not measure sound and cannot test multi-year traffic trends. The district's five-year operations summaries (roughly 55 percent piston, 17 percent jet per the district's [2024 annual operations report](https://truckeetahoeairport.com/board_meetings/370/view_file?file=tab+10b+-+2024+annual+operations+and+comments+report.pdf)) are the source for long-run mix and trend questions.
